...
const CHANNELS = [
  ...
  },
  {
    ...
    icon: (
      <>
        <circle cx="20" cy="20" r="15" fill="none" stroke="currentColor" strokeWidth="2.3" />
        <path
          d="M20 11v9l6 4"
          fill="none"
          stroke="currentColor"
    ...
    icon: (
      <>
        <path
          ...
          strokeLinejoin="round"
        />
        <circle cx="20" cy="16" r="4.4" fill="none" stroke="currentColor" strokeWidth="2.3" />
      </>
    ),
  },
]
...
export default function Contact() {
  ...
  const fieldCls = (err?: string) =>
    `w-full rounded-xl border bg-cream/55 px-4 py-3 text-[0.95rem] text-ink outline-none transition-colors duration-200 placeholder:text-muted/70 focus:bg-paper ${
  ...
  return (
    ...
    >
      ...
      <div
        className="pointer-events-none absolute -left-[8%] bottom-[6%] h-[38vw] max-h-[480px] w-[38vw] max-w-[480px] rounded-full bg-gold/10 blur-[130px]"
        aria-hidden
      />
      ...
      <div className="relative mx-auto max-w-[1280px] px-5 sm:px-8">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-14">
          ...
          <div className="lg:col-span-5">
            ...
            <h2
              className="display-kr mt-6 text-[clamp(2rem,4vw,3.2rem)] leading-[1.3] text-ivory"
              data-reveal
            ...
            <p
              className="mt-7 max-w-[27rem] text-[0.99rem] leading-[1.95] text-ivory/65"
              data-reveal
              style={{ '--reveal-delay': '150ms' } as React.CSSProperties}
            >
              투자에 대한 어떤 고민이든 편하게 문의해 주세요. 원타임 그룹의
              전문 컨설턴트가 정성껏 안내해 드립니다.
            </p>
            ...
            <div className="mt-10 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
              {CHANNELS.map((c, i) => (
                <div
                  key={c.label}
                  className="flex items-center gap-4 rounded-xl border border-ivory/12 bg-navy-soft/60 p-4"
                  data-reveal
                  style={
                    { '--reveal-delay': `${i * 90}ms` } as React.CSSProperties
                  }
                >
                  <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-gold/15 text-gold-soft">
                    <svg viewBox="0 0 40 40" className="h-[22px] w-[22px]">
                      {c.icon}
                    </svg>
                  </span>
                  <span className="min-w-0">
                    <span className="block font-sans text-[0.68rem] font-semibold tracking-[0.2em] text-gold-soft/80">
                      {c.label}
                    </span>
                    <span className="mt-0.5 block truncate text-[0.95rem] font-medium text-ivory">
                      {c.value}
                    </span>
                  </span>
                </div>
          ...
          >
            <div className="rounded-2xl bg-ivory p-7 text-ink sm:p-10">
              ...
              <form className="mt-6 space-y-4" onSubmit={handleSubmit} noValidate>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    />
                    {errors.name && (
                      <p className="mt-1.5 text-[0.78rem]" style={{ color: ERR }}>
                        {errors.name}
                      </p>
                    )}
                  </div>
                ...
                <div>
                  <label className="flex cursor-pointer items-start gap-2.5 text-[0.86rem] text-ink-soft">
                    ...
                    <span>
                      ...
                      목적으로만 이용됩니다. <span className="text-gold-deep">*</span>
                    </span>
                  </label>
                  {errors.agree && (
                    <p className="mt-1.5 text-[0.78rem]" style={{ color: ERR }}>
